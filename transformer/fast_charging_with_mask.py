# -*- coding: utf-8 -*-
"""fast_charging_with_mask.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1AaL9xJgRfz_iK3WNvkgFy22ZpaXUfSPL
"""
...
class Dataset_battery(Dataset):
    def __init__(self, flag='train', size=None,
                 ...
                 target='qc', scale=True, inverse=False, timeenc=0, freq='h', cols=None):
    ...
    def __read_data__(self):
        ...
        df_raw = pd.read_csv('/content/sample_data/ChargeProfileIdeal.csv')
        mask = df_raw['mask']
        ...
        if self.cols:
            ...
            cols.remove('cycle')
            cols.remove('mask')
        df_raw = df_raw[['cycle']+cols+[self.target]]
        ...
        mymask = mask.values
        df_stamp = df_raw[['cycle']][border1:border2]
    ...
    def __getitem__(self, index):
        ...
        # print("Shape of x ",seq_x.shape)
        # # [150,1]
        # print("Shape of y ",seq_y.shape)
        # # [200,2]
        # print("Shape of x_mark ",seq_x_mark.shape)
        # print("Shape of y_mark ",seq_y_mark.shape)
        # print("shape is",mask_x.shape)
        # print("shape is",mask_y.shape)
...
class Dataset_Pred(Dataset):
    def __init__(self, flag='pred', size=None,
                 ...
                 target='qc', scale=True, inverse=False, timeenc=0, freq='15min', cols=None):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24*4*4
    ...
    def __read_data__(self):
        self.scaler = StandardScaler()
        df_raw = pd.read_csv('/content/sample_data/TestProfile.csv')
        ...
        tmp_stamp = df_raw[['cycle']][border1:border2]
        ...
        data_stamp = time_features(tmp_stamp, timeenc=self.timeenc, freq=self.freq[-1:])
    ...
    def __getitem__(self, index):
        ...
        if self.inverse:
            ...
            mask_x = self.mask[s_begin:s_end]
            mask_y = self.mask[r_begin:r_begin+self.label_len]
        seq_x_mark = self.data_stamp[s_begin:s_end]
        seq_y_mark = self.data_stamp[r_begin:r_end]
        # print("Shape of x ",seq_x.shape)
        # # [96,7]
...
class FullAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, x_mask, attn_mask):
        ...
        # print("What is B and L , h , e ",B," ",L," ",H," ",E) 6, (200,100,50), 2 , 100
        # print(x_mask.shape)
        scores = torch.einsum("blhe,bshe->bhls", queries, keys)
        if self.mask_flag:
            if attn_mask is None:
                attn_mask = TriangularCausalMask(B, L, x_mask, device=torch.device('cuda:0'))
            # print(attn_mask.mask.shape) 6, 1 , 150,150
...
class Exp_Transformer(Exp_Basic):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x,batch_y,batch_x_mark,batch_y_mark,bat_mask,bat_masy) in enumerate(train_loader):
                ...
                # print(bat_mask.shape)
                # 32 , 72 , 7
                # print(bat_masy.shape)
                # 32 , 72 , 4
    ...
    def predict(self, setting, load=False):
        ...
        for i, (batch_x,batch_y,batch_x_mark,batch_y_mark,bat_mask,bat_masy) in enumerate(pred_loader):
            pred, true = self._process_one_batch(
                pred_data, batch_x, batch_y, batch_x_mark, batch_y_mark,bat_mask,bat_masy)
            preds.append(pred.detach().cpu().numpy())
    ...
    def _process_one_batch(self, dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark, bat_mask, bat_masy):
        ...
        # print("We have come pretty deep output",outputs.shape)     [6, 50, 2]
        # torch.Size([32, 24, 7])
        # print("We have come pretty deep batY",batch_y.shape)       [6, 150, 2]
        # torch.Size([32, 72, 7])
...
args = ArgsNamespace(model='Transformerstack', data='battery', features='MS',
                     seq_len=200, label_len= 100, pred_len = 50,
                     d_model=200, n_heads= 2, e_layers = 2,
                     d_layers=1, d_ff= 2048, attn = 'full',
                     factor=5, embed= 'timeF', distil = True,
                     mix=True, des= 'test',
                     enc_in=2, dec_in= 2, dropout = 0.05,
                     activation='gelu', freq = 'h', batch_size = 6, patience = 3,
                     learning_rate = 0.001, use_amp = False, use_multi_gpu = False, gpu = 0,s_layers = [3,2,1],
                     c_out = 2, output_attention = False, target = 'qc',inverse = False, cols = None,
